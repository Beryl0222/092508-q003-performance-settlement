...
class Store:
    ...
    def add(self, record: Record) -> Record:
        ...
        with self.connection:
            self.connection.execute(
                "INSERT INTO performance_run(record_id, owner_id, state, revision, created_at) VALUES(?,?,?,?,?)",
    ...
    def get(self, record_id: str) -> Record | None:
        row = self.connection.execute(
            "SELECT record_id, owner_id, state, revision, created_at FROM performance_run WHERE record_id=?",
            (record_id,),
        ).fetchone()
